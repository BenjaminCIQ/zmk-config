...
This fork adapts urob's config for the [Dasbob](https://github.com/GroooveBob/DASBOB) 36-key board and adds:

- 36-key [Dasbob](https://github.com/GroooveBob/DASBOB) support for `nice_nano@2.0.0`, including left/right shields and settings reset.
- A custom Workman-flavoured 36-key base layout.
- A generated swap-hands layer powered by [`scripts/generate_mirror.py`](scripts/generate_mirror.py), including HRM handedness swaps.
- A Keymap Editor-compatible expanded keymap generated by [`scripts/expand_keymap.py`](scripts/expand_keymap.py), with the sync expansion workflow documented in [`config/DASBOB.md`](config/DASBOB.md) to help compare and port upstream changes from urob's main config.
- Autoshift punctuation on the fork’s punctuation keys.
- User-specific combos for symbols, editing, leader activation, mouse activation, brackets/braces/parens, and backspace/delete.
